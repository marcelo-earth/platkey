 ...
 */
function getContributionId(author: string, url: string, textPreview: string) {
 ...
 * @returns {boolean}
 */
function deleteContributionById(contributionId: string, callback = () => {}) {
  chrome.storage.sync.get("savedContributions", ({ savedContributions }) => {
    let updatedSavedContributions = savedContributions.filter(
      (saveContribution: any) => saveContribution.id !== contributionId
    );
...
if (window.location.href.startsWith("https://platzi.com/clases/")) {
  function saveClass() {
    ...
    chrome.storage.sync.get("classlist", ({ classlist }) => {
      let isClassHighlighted = classlist.some(
        (classitem: any) => classitem.url === window.location.href
      );
   ...
   * @param {Event} event
   */
  function saveContribution(event: MouseEvent) {
    const saveContributionButton = event.target as HTMLButtonElement;
    const contributionId = getContributionId(
      saveContributionButton.getAttribute("data-author") as string,
      saveContributionButton.getAttribute("data-link") as string,
      saveContributionButton.getAttribute("data-textpreview") as string
    );
  ...
  const loadSaveContributionButtons = () => {
    ...
    for (let index = 0; index < contentAuthorElements.length; index++) {
      ...
      const contributionTextElement = contentAuthorElement.parentNode
        .children[1].children[1].children[0] as any;
  ...
  chrome.storage.sync.get("classlist", ({ classlist }) => {
    ...
    highlightClassButton.appendChild(saveClassIconSVG as ChildNode);
...
if (window.location.href === "https://platzi.com/home") {
  ...
  const loadPlatziHighlights = () => {
    ...
    chrome.storage.sync.get("classlist", ({ classlist }) => {
      classlist.forEach((classelement: any) => {
        ...
        classItems.appendChild(htmlToElement(classItem) as ChildNode);
    ...
    function deleteContribution(event: any) {
      ...
      if (confirm) {
        ...
        const contributionCard = document.getElementById(
          `ContributionCard-${contributionId}`
        ) as HTMLElement;
    ...
    chrome.storage.sync.get("savedContributions", ({ savedContributions }) => {
      savedContributions.forEach((savedContribution: any) => {
        ...
        platziListContributions.insertAdjacentElement(
          "beforeend",
          classItemsContributions
        );
        studentHomeWrapperContributions.insertBefore(
          platziListContributions,
          learningPathsListContributions
        );
      });
    });
  };
